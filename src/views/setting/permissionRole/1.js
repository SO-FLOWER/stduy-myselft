...
//   通过递归调用一步步更新格式 再删除之前的格式 有需要的可以参考下 
    ...
    componentDidMount(){
       ...
       this.setState({
        ...
        createTime: this.props.msg.createTime,
    ...
    clickNav1=(roleId,deptId,roleName,type,e)=>{
        ...
        if(type=='set'){
            this.props.history.push({ pathname: `/PermissionRole/permissionSet/${ roleId }`, state: {roleId: roleId}})
        }else{//人员授权
            this.props.history.push({ pathname: `/PermissionRole/userAgree/${  roleId }`, state: {roleId: roleId }})
        }
            ...
            <Fragment> 
                ...
                this.state.type=='add'?(
                    <Fragment >
                     ...
                     <label style={{width : '100px'}}>{this.state.roleName}</label>
                    <label style={{width : '210px'}}>{this.state.deptName}</label>
                    <label style={{width : '200px'}}>{this.state.createBy}</label>
                    <label style={{width : '200px'}}>{this.state.createTime}</label>
...
||||||| .r2972
                    ...
                    {/* <SelectComponet departmenList={depart} type={'depart'}/>  */}
=======
                                    // showIcon={false}
                                    // defaultExpandAll
                                    // onSelect={this.onSelect}
