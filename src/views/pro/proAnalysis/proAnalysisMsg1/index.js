...
import './index.less';
import Msg from './../proAnalysisMsg/index.js';
...
class ProAnalysis extends Component {
    ...
    state={
        ...
        maxNum :this.props.maxLength*1+1,
        showChidren:false,
        allTimeList:[],
        userManhourList:[], 
    }
    ...
    render(){
        ...
        let picList = '';
        if(this.props.msg.avatarList.length > 0){
        ...
        if(this.state.allTimeList.length > 0){
            allTimeList = this.state.allTimeList.map((val,key)=>{
                 return <Msg key={key} msg={val} level={num+1} dataMsg={this.props.dataMsg} maxLength={this.state.maxNum}/>
            })
        }
        let userManhourList = '';
        ...
        console.log('1',this.state.maxNum)
        return (
            <Fragment>
         
                                <div className='tableMsg1'>
        <label onClick={this.changeDept}><span style={{display:'inline-block',width:num*10+'px',height:'10px'}}></span>{!this.state.showChidren?<RightOutlined />:<DownOutlined/> }{this.props.msg.name}</label>
